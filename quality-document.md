...
- Basis: deterministic Python suite, compile checks, Swift client tests, iOS simulator build evidence, WebSocket simulator verification, RunPod real-ASR proof paths, Modal NeMo FastConformer scoped Surah 108 live replay proof, documented R2/GitHub bootstrap flow, and source/build/runtime-smoke/scored-fixture/app-replay/local-locator/Tanzil-loader/app-bundle/app-binary-replay plus live mic capture/replay, iOS Simulator invalid-output diagnosis, actionable failure UI, shared physical-device replay scheme/build proof, fresh-install CoreML default wiring, selected-Surah ordered CoreML locator state, selected-Surah bounded initial matching, selected-Surah current-ayah completion-gated post-lock progression, selected-Surah guarded long-current-ayah next-ayah tail-skip fallback for the physical-iPhone Surah 47:2 -> 47:3 shape, selected-Surah no-hidden-advance prefix/sequence locks, selected-Surah short fused-opening lock for physical-iPhone Surah 80 startup, selected-Surah current-ayah final-word recovery for physical-iPhone Surah 80 ayah 4, selected-Surah istiaza/basmala opening lock, selected-Surah unresolved-istiaza startup guard and sparse first-ayah recovery, selected-Surah noisy initial anchor lock, selected-Surah prefix-span first lock for cumulative basmala starts, selected-Surah sequence-anchor lock for skipped short middle ayahs, selected-Surah ordered-anchor post-lock recovery for omitted expected-ayah openings, selected-Surah short-ayah suffix recovery, bounded post-lock transcript matching without later-ayah skips, speech-boundary/post-transcript blank-streak CoreML acoustic stream reset, canonical word highlighting with stable located-ayah/next-expected UI diagnostics, local locator/audio-window/latency diagnostics, fixed-divisor live chunk cadence for the experimental local CoreML FastConformer path, physical-iPhone Surah 59 bounded-latency evidence, and asset-catalog packaging for the Quran logo named image.

The MVP has strong harness discipline and a working real-ASR mobile path, but it still needs longer-session proof, broader physical-device validation, latency tuning, and more polished ordered-progression guidance.
...
| Mobile UX readiness | MVP | Simulator path works and exposes latest ayah, next expected ref, and ayah text status. The local CoreML preset is selectable and has automated fixture app-replay proof, a Tanzil-format local corpus loader, local app-bundle copy paths, stable located-ayah headline behavior, and macOS app-binary replay proof, but physical iPhone visual confirmation, live microphone latency, and guidance copy need more work. |
...
- Deployed Modal L4 endpoint `https://moabdelmoez--tarteel-realtime-asr-fastapi-app.modal.run`; `/ping` returned `{"status":"ok"}` and logs showed the NeMo model restored from the nested checkpoint.
- Scoped Surah 108 app-cadence replay with `--chunk-ms 160 --send-speech-end` locked all three local short fixtures: `108001.wav -> 108:1`, `108002.wav -> 108:2`, and `108003.wav -> 108:3`.
- Manual macOS app replay through the Modal WSS URL locked `108001.wav` to `108:1`; iOS Simulator build passed, but manual Simulator install/launch control hung and produced a blank screenshot, so iOS manual ASR proof remains open.
- Verification passed through focused Python ASR/runtime/WebSocket checks, full Python suite with 312 tests, compileall, Swift client core with 34 XCTest plus 88 Swift Testing tests, and macOS/iOS app builds.
- Release posture: materially stronger Modal evidence for short scoped Quran ASR, but still a prototype proof rather than broad live-recitation quality or provider-cost readiness.

### 2026-06-13 - Selected-Surah CoreML Surah 47 Tail-Skip Recovery

- Diagnosed `/private/tmp/tarteel-iphone-coreml-20260613-150653.logarchive`: the 15:00 physical-iPhone Surah 47 run reached late `47:2` with `next_expected_ref=47:2:14`, then stayed `coreml_local_ordered_no_match` despite clear `47:3` transcript anchors.
- Added `coreml_local_next_ayah_tail_skip_progress`, a selected-Surah-only fallback that runs after current-ayah matching fails, considers only the immediate next ayah, and requires a long mostly-complete current ayah with a small remaining tail plus strong next-ayah anchors near the start.
- Verification passed through red/green Surah 47 Swift regression, `CoreMLFastConformerTests` with 52 Swift Testing tests, full Swift client core with 34 XCTest plus 85 Swift Testing tests, and iOS Simulator `TarteelPrototypeCoreMLReplay` build.
- Release posture: improves the concrete 47:2 -> 47:3 locator policy gap without reopening broad later-ayah skips; physical-device retest is still required.

### 2026-06-13 - Physical-iPhone CoreML Log Follow-Up And Logo Asset Packaging

- Diagnosed `/private/tmp/tarteel-iphone-coreml-20260613-143519.logarchive`: June 13 selected-Surah 59 loaded the CoreML model and Tanzil corpus, progressed to `ayah_ref=59:10` / `next_expected_ref=59:11:1`, and had zero locator windows over 1s.
- Compared the older June 12 locator backlog signature: max `locator_ms` was around 4774.6 with 4 windows over 1s, matching the already-covered bounded selected-Surah latency guardrails rather than a new inference freeze.
- Fixed the remaining SwiftUI named-image warning by packaging `quran_logo` as an asset-catalog imageset as well as a copied bundle resource.
- Verification passed through red/green Apple project guardrail, focused Apple source guardrails, focused Swift locator timing tests, full Swift client core, and iOS/macOS app builds.
- Release posture: physical-device locator latency looks healthy for the latest Surah 59 run, and the app should stop warning about `quran_logo`; ASR quality remains experimental.

### 2026-06-12 - CoreML Located-Ayah UI Visibility

- Diagnosed the latest physical-iPhone Surah 80 archive as a visibility problem, not a missing locator event: the fresh run reached `80:4` and the UI reducer reported `state_changed=true`.
- Updated shared state so progress events show `Ayah <ref>` and post-progress locating/no-match packets preserve that located headline instead of reverting immediately to generic `Listening`.
- Added `debugNextExpectedText`; iPhone and macOS status panels now show `Latest ayah` and `Next expected`, and the macOS share summary includes `Next expected`.
- Verification passed through red/green Swift state tests, focused Apple source guardrails, full Swift client core with 34 XCTest plus 84 Swift Testing tests, `git diff --check`, and macOS/iOS app builds.
- Release posture: improves manual-test/user visibility when short ayahs are recognized; it does not improve CoreML ASR quality or the noisy post-80:4 gap before 80:5.

### 2026-06-12 - Selected-Surah CoreML Final-Word Recovery

- Diagnosed fresh physical-iPhone Surah 80 logs: fused startup lock and ordered progress reached `next_expected_ref=80:4:1`, but ASR mostly preserved only the final word `الذكرى` for ayah 4.
- Added a narrow selected-Surah post-lock `coreml_local_short_ayah_final_word_progress` path for the current expected four-word ayah when its distinctive final word is strongly recognized.
- Kept broad one-word suffix acceptance disabled; the existing Surah 18 one-word suffix no-match behavior remains covered.
- Verification passed through red/green physical-iPhone-shaped Surah 80 ayah 4 regression, focused CoreML tests with 51 Swift Testing tests, and full Swift client core with 34 XCTest plus 82 Swift Testing tests.
- Release posture: better physical-iPhone Surah 80 ordered progression when ASR clips the middle of a short ayah, while ASR blank/noisy output remains a separate decoder/preprocessing problem.

### 2026-06-12 - Selected-Surah CoreML Fused Opening Lock

- Diagnosed physical-iPhone CoreML logs as mixed but valuable: Surah 59 tracked well on real hardware, Surah 80 failed before first lock from fused/sparse startup ASR, and Surah 104 remained mostly blank/unusable ASR.
- Added a narrow selected-Surah `coreml_local_opening_fused_lock` path for short first ayahs beginning with basmala, using compact opening and content evidence without broad selected-Surah relocking.
- The first implementation was tightened after full CoreML tests caught an unsafe Surah 18 startup lock; longer first ayahs stay on the existing bounded path.
- Verification passed through red/green physical-iPhone-shaped Surah 80 regression, focused CoreML tests with 50 Swift Testing tests, full Swift client core with 34 XCTest plus 81 Swift Testing tests, Apple/CoreML Python guardrails with 26 tests, and macOS/iOS app builds.
- Release posture: better physical-iPhone Surah 80 startup resilience, while Surah 104-style blank ASR remains an audio/model/decoder investigation rather than a locator claim.
