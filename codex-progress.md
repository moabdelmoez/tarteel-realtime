...
- Completed:
  ...
  - Fixed stale-generation follow-ups so delayed audio send failures and old socket callbacks cannot mutate a stopped or newer recording session.
...
- Verification run:
  ...
  - Stale-generation regression tests failed before the fix: a delayed old send failure stopped the restarted session, and an old socket event mutated stopped state.
